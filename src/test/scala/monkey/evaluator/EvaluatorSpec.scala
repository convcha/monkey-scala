...
import monkey.`object`.{MArray, MBoolean, MError, MFunction, MHash, MInteger, MString}
import monkey.evaluator.Evaluator._
...
class EvaluatorSpec extends FlatSpec with Matchers {
  ...
  "A Evaluator" should "handle the error" in {
    ...
    tests.foreach {
      case (input, expectedMessage) => {
        ...
        val errObj = evaluated.asInstanceOf[MError]
  ...
  "A Evaluator" should "evaluate Function object" in {
    ...
    val fn = evaluated.asInstanceOf[MFunction]
    ...
    tests.foreach {
      case (input, expected) => {
        ...
        expected match {
          ...
          case _: String => {
            evaluated shouldBe a[MError]
            evaluated.asInstanceOf[MError].message shouldBe expected
          }
        }
      }
    }
  ...
  "A Evaluator" should "evaluate Array literals" in {
    ...
    val result = evaluated.asInstanceOf[MArray]
  ...
  "A Evaluator" should "evaluate Hash literals" in {
    ...
    val evaluated = testEval(input)
    evaluated shouldBe a[MHash]

    val result = evaluated.asInstanceOf[MHash]

    val expected = Map(
      MString("one").hashKey   -> 1,
      MString("two").hashKey   -> 2,
      MString("three").hashKey -> 3,
      MInteger(4).hashKey      -> 4,
      TRUE.hashKey             -> 5,
      FALSE.hashKey            -> 6,
    )
  ...
  def testEval(input: String): MObject = {
    val l       = Lexer(input)
    val p       = Parser(l)
  ...
  def testIntegerObject(obj: MObject, expected: Long): Unit = {
    obj shouldBe a[MInteger]
    val result = obj.asInstanceOf[MInteger]
    result.value shouldBe expected
  }

  def testBooleanObject(obj: MObject, expected: Boolean): Assertion = {
    obj shouldBe a[MBoolean]
    val result = obj.asInstanceOf[MBoolean]
    result.value shouldBe expected
  }

  def testNullObject(obj: MObject): Assertion = {
    obj shouldBe NULL
  }
}
